// Format-only normalization of approved imagegen animation sheets: preserve RGBA,
...
const source = path.join(root, 'ArtSource/HeroExternalVfx/raw');
const dest = path.join(root, 'Assets/Resources/Effects/HeroExternal');
const review = path.join(root, 'docs/validation/hero_external_vfx');
const ids = ['sword_qi', 'palm_force', 'heavy_cleave'];
...
(async () => {
  ...
  for (let row = 0; row < ids.length; row++) {
    ...
    const name = 'spr_vfx_hero_' + id + '_6f_v01.png';
    await sharp({create: {width: 1536, height: 256, channels: 4, background: '#00000000'}})
      .composite(frames).png().toFile(path.join(dest, name));
    report.clips.push({id, source: 'ArtSource/HeroExternalVfx/raw/' + id + '.png', output: name, frames: stats});
  }
  for (const [name, color] of [['contact_dark', '#171e22'], ['contact_light', '#d3c8b2']])
    await sharp({create: {width: 1536, height: 768, channels: 4, background: color}})
      .composite(contact).png().toFile(path.join(review, name + '.png'));
  for (let i = 0; i < cycles.length; i++)
    await sharp({create: {width: 768, height: 256, channels: 4, background: '#171e22'}})
  ...
  animation.push(await sharp({create: {width: 768, height: 256, channels: 4, background: '#171e22'}}).raw().toBuffer());
  await sharp(Buffer.concat(animation), {raw: {width: 768, height: 256 * 7, channels: 4, pageHeight: 256}})
  ...
  console.log('Packed three transparent six-frame strips; common center/scale, no clipping, no duplicate frames.');
