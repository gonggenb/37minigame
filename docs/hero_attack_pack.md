...
## A1 / B1 / C1 正式外放帧动画（2026-09-24）

用户选定的月牙剑气、前冲掌劲和赤金斜劈现已生成三套六帧透明图集并接入 Unity。
正式帧图替换外放程序弧线和实体掌印；保留上轮收束后的释放/收尾时序、发射点锁定和高速适配。
资源、逐帧预览、实际横竖屏证据及运行入口见 [外置帧动画接入](validation/hero_external_vfx/README.md)。

## 外放特效收束（2026-09-24）

剑气、掌印和重斩外放层现在随本次动作缩短：动作 22% 时释放，94% 前结束；取消固定 0.68 秒尾效。
剑气保留一道紧凑飞行弧，掌法缩小掌印和命中圈，重斩缩小贴地冲击与裂纹。发射后固定起点，
目标仍跟随敌人；屏幕尺寸变化时舍弃旧布局的外放层。既有贴身层、普通攻击图内效果和大招不变。
Unity 编译及 715 项 Play Mode 检查通过，包括横竖屏、普通/高速动作结束清理及三条时间规则。
检查证据与复现入口见 [外放特效收束](validation/combat_vfx_cohesion/README.md)。

## 武学贴身特效（2026-09-21）

剑气、掌法与重斩新增跟随人物当前动作的自身特效。使用现有像素帧残影、绕身气流、掌心聚气、
贴身剑光与脚底蓄势环；破甲/毒掌和惊鸿/血气按本次真实出招快照区分金、绿、红色。
该层随当前动作消失，不积压高速出手；普通攻击继续使用三版图内特效。
原有飞行与敌方命中特效保留，伤害、攻速和计时不变，无需手动绑定。
文件列表与验证说明见 [主角武学贴身特效](validation/hero_self_vfx/README.md)。

## 三版随机普攻（2026-09-21）

普通攻击已接入用户选定的青白剑罡、赤金烈斩、墨影残像三套自带特效的八帧动画。
每次普通出手等概率随机选择一套，整次锁定，允许连续重复。武学专属动作保持原优先级；
动画随机使用独立随机数，不消耗玩法随机状态。起始青钢剑每三次命中触发的剑气仍保留。
三张新增图集使用现有 Resources 与导入器，无需手动绑定，旧普攻保留作待机种子和资源回退。
处理脚本、文件列表、播放预览与验证入口见 [随机普攻记录](validation/hero_basic_variants/README.md)。
...
以下为早期接入记录；其中固定时长、尾弧及扩散范围已由上方 2026-09-24 收束调整替代。
...
五种流派绝学达到最高重数后，在其原有机制实际发动时播放高规格专属演出：七剑、毒莲、金钟、月影、血轮。
演出 1.30 秒，共用每场 4 秒视觉间隔；满重普通武学不触发。战斗照常结算，不暂停或改变三条核心计时规则。
素材、实现、真实触发点、文件清单和横竖屏验证见 [满重绝学说明](validation/hero_ultimates/README.md)。

## 大招角色帧与帧内特效

五种满重流派绝学现各有完整八帧角色动作，蓄力、出手、回收的特效直接烘焙在角色帧中。
角色动作与原外围法相共用 1.30 秒演出时钟，完整播放期间不被普攻和敌方动作覆盖，结束后回到当前普通动作/待机。
资源、完整提示词、归一参数和验证见 [大招角色动作](validation/hero_ultimate_poses/README.md)。
