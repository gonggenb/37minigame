...
教学关卡使用独立的 30 秒时限；关卡2仍使用 60 秒主地图规则。完成或跳过教学后，流程先在教学场景
停在不可操作的 `Ready` 状态显示“难度飙升！！！”弹窗，关闭后才加载关卡2并进入原有起手武学选择。
弹窗沿用教学开场的遮罩和卡片组件，每次从教学衔接关卡2时显示；直接从关卡选择进入关卡2时不显示。
教学 30 秒后先进入新手守关说明，确认后用基础自动战斗挑战低难度 Boss；胜利后进入场景内的 `Result` 总结状态；选择“下一关”才发出关卡2自动启动请求。
正式关卡的 Boss 胜负也进入同一总结 UI。当前场景序列没有关卡3，因此关卡2总结中的下一关入口禁用。
...
- 当前攻击或战力可通过角色按钮查看。
- 当前铜钱或修为。
- 当前主要武学：竖屏探索默认收进角色面板，战斗保留武学栏；横屏维持既有布局。
