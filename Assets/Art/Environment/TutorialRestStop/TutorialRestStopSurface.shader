Shader "Wuxia Roguelite/Tutorial Rest Stop Surface"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        half _Emission;
        half _Foliage;
        float4 _RestStopPlayerPosition;
        struct Input { float4 color : COLOR; float3 worldPos; float4 screenPos; };
        void surf(Input IN, inout SurfaceOutputStandard o)
        {
            if (_Foliage > 0.5 && _RestStopPlayerPosition.w > 0.5)
            {
                float3 player = _RestStopPlayerPosition.xyz + float3(0,1,0);
