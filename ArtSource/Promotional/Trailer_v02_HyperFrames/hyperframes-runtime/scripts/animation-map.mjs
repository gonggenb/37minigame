...
// Compatibility adapter: the npm CLI bundles producer rather than exposing it.
// Reuse the skill's tween analysis with local Playwright for GSAP-only mapping.
import { chromium } from '/Users/gongyuyang/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/node_modules/playwright/index.mjs';
async function createFileServer({ projectDir }) { return { url: 'file://' + join(projectDir, 'index.html'), close() {} }; }
async function createCaptureSession(url, out, opts) {
...
async function closeCaptureSession(s) { await s.browser.close(); }
async function getCompositionDuration(s) { return s.page.evaluate(() => Number(document.querySelector('[data-composition-id]').dataset.duration)); }
