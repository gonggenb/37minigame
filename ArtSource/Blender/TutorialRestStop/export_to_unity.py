...
Run using Blender --background TutorialRestStop_v01.blend --python export_to_unity.py.
...
OUT=ROOT+'/Assets/Art/Environment/TutorialRestStop'
os.makedirs(OUT,exist_ok=True)
source=bpy.data.scenes['Level 01 | 山脚古道的小歇脚地']
export=bpy.data.scenes.new('TutorialRestStop_UnityExport')
...
def portable_material(mat):
 ...
 index=len(materials); name='TR_M%02d'%index
...
for original in source.objects:
 ...
 if not group or int(group[:2])>24 or original.type not in {'MESH','FONT'}:continue
 ...
 if source_material and any(k in source_material.name for k in ['leaf','flower']):
  bpy.context.view_layer.objects.active=ob
  modifier=ob.modifiers.new('Gameplay leaf simplification','DECIMATE');modifier.ratio=.65
 ...
 for p in mesh.polygons:
  if int(group[:2]) in [20,21,22,23,24]:
   center=sum((mesh.vertices[i].co for i in p.vertices),Vector())/len(p.vertices)
   cell=(math.floor(center.x/8),math.floor(center.y/8))
  else:
   center=sum((mesh.vertices[i].co for i in p.vertices),Vector())/len(p.vertices)
   # Separate the reward prop from supplies, so consumption can hide only the chest.
   if int(group[:2])==7:cell=('chest' if abs(center.x+5.2)<.85 and abs(center.y-4.33)<.65 else 'supply',0)
   else:cell=(0,0)
  partitions[cell].append(p)
 for part,(cell,polys) in enumerate(partitions.items()):
  name='TR_%s_%03d_%02d'%(group[:2],len(stats),part)
  remap={}; verts=[];faces=[]
  for p in polys:
   face=[]
   for i in p.vertices:
  ...
  stats.append({'name':name,'group':int(group[:2]),'vertices':len(verts),'polygons':len(faces),'source_object':original.name,'role':str(cell[0])})
 bpy.data.objects.remove(ob,do_unlink=True)

for original in source.objects:
 if original.type=='EMPTY' and original.name.startswith('Anchor_'):
  ob=bpy.data.objects.new(original.name,None);export.collection.objects.link(ob);ob.location=original.location
bpy.ops.object.select_all(action='SELECT')
bpy.ops.export_scene.fbx(filepath=OUT+'/TutorialRestStop.fbx',use_selection=True,object_types={'MESH','EMPTY'},axis_forward='-Z',axis_up='Y',apply_unit_scale=True,bake_space_transform=True,use_mesh_modifiers=True,mesh_smooth_type='FACE',add_leaf_bones=False,bake_anim=False,path_mode='AUTO')
with open(OUT+'/TutorialRestStopExport.json','w') as f:json.dump({'materials':manifest,'meshes':stats,'coordinate_system':'Imported FBX anchors are (-X,Z,-Y); prefab Y=180 restores (X,Z,Y)','source':'ArtSource/Blender/TutorialRestStop/TutorialRestStop_v01.blend'},f,indent=2)
