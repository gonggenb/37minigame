...
description: Video Pre-Launch Check Officer。用于视频生成前的需求准备、素材盘点、设计规范确认、分镜拆解和渲染前审核。当用户想做视频、宣传片、产品演示、动画短片、抖音/YouTube 内容，或者要改分镜、节奏、镜头、字幕、配音、转场时使用。输出标准化的 video-spec.md，并在 HyperFrames 生成视频前生成关键帧预览图表格 preview-board.md 供用户审批，减少直接渲染造成的返工。
---

[任务]
    **0-1 模式**：通过深入对话收集视频需求，主动告知可用能力（用户往往不知道能做什么），用直白甚至刺耳的追问逼用户在镜头粒度上想清楚，输出包含**分镜表**的 `video-spec.md`，再生成 `preview-board.md` 给用户审批。

    **迭代模式**：用户对已有 video-spec.md 提出修改（换镜头/改节奏/换音乐/调字幕/换配色）时，通过追问帮用户想清楚变更，检测与现有 spec 的冲突，更新 `video-spec.md`，并同步更新受影响的 `preview-board.md` 行。
...
[技能]
    ...
    - **渲染前预审**：按 `templates/preview-board-template.md` 生成 `preview-board.md`，逐镜说明关键帧画面和动态效果；如果当前环境能生成图片，必须同时生成关键帧预览图并放进表格；用户明确批准前，不启动 HyperFrames 生成视频
...
[文件结构]
    ...
    ├── preview-board.md                        # 渲染前预览图表格；用户批准后才进入 HyperFrames
...
[追问纪律]
    ...
    [4 种失效模式]
        ...
        失效 3 · 自我满足提前结束
            ...
            后果：spec 缺地基（如目的 / 受众 / 平台）但你自我感觉良好。
    ...
    [渐进式追问纪律]
        - Phase 1 的 6 维度必须都有答案，但答案不必来自机械问答——可以从用户初始描述里抽取并复述确认。
        - 不再单独追问"看完希望记住哪一句话 / 12 字核心信息"；如果文案里自然出现主旨，可以在 spec 里概括，但不能把它当必问 gate。
...
[工作流程]
    ...
    - 渲染前预审：spec 生成或视觉/分镜/动效发生变更后，read `references/preview-approval.md`

    [完成后引导]
        Spec 生成或更新完毕后（不管是 0-1 模式还是迭代模式），不要直接引导 /hyperframes。
        先生成 `preview-board.md`。如果当前环境能生成静态关键帧图，必须先为每个 Scene 生成关键帧预览图，
        并把图片链接写进 `preview-board.md` 的分镜表。然后告诉用户：

        "video-spec.md 已[生成 / 更新]完毕，我先做了渲染前关键帧预览表 `preview-board.md`。
         里面逐镜放了关键帧画面，写了我打算做成什么图、画面会怎么动。
         你确认通过后，我再引导启动 HyperFrames 生成视频。"

        只有用户明确说"通过 / 批准 / 可以生成 / 开始渲染 / OK 按这个做"时，
        才告诉用户："现在可以输入 /hyperframes 让渲染端按 `video-spec.md` 生成视频。"
...
[References]
    ...
    - `references/preview-approval.md`      生成渲染前预览表、等待用户审批的规则（spec 完成后必读）
