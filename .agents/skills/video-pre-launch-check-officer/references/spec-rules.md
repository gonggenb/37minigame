...
- `video-spec.md` 完成后还不能直接渲染，必须先生成 `preview-board.md` 让用户审批
...
- 不单独要求「核心信息 / takeaway / 12 字金句」字段；不要为了填字段回头追问用户"看完要记住哪句话"
...
- 示例：`视频基本盘 / 目的` · `Scene 03 / 画面描述` · `音频时间轴 / 背景音乐`
- 无开放问题写「无，spec 已完整」
...
- [ ] 已准备生成 `preview-board.md`，并且不会在用户批准前引导 `/hyperframes`

---

## Preview Board 一致性校验

生成 `preview-board.md` 前逐条过一遍：

- [ ] 每个 Scene 都有一行，Scene 编号和时间与 `video-spec.md` 完全一致
- [ ] 「简单介绍」不是复制旁白，而是解释这一镜在做什么
- [ ] 「预览图计划」能让用户想象静帧长相：主体、构图、文字、背景、关键素材都清楚
- [ ] 「动态效果计划」能让用户想象画面怎么动：出现、推进、强调、转场都清楚
- [ ] 「风险 / 待确认」列出会导致返工的点；没有就写「无」
- [ ] 用 `broll-abstract.placeholder` 的 Scene 已在风险里标出
- [ ] 审批状态是「待用户批准」，没有用户明确批准前不改成通过
