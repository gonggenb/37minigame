...
[输入形态]
    ...
    4. 都没有
        基于视频目的 + 受众 + 时长，你写草稿（前置：Phase 1 基本盘齐全）

[输入校验]
    每种输入都必须能回答：
    - 总时长（来自 Phase 1）
    - 信息密度档位（hook / 教程 / 纪录）
    - 视频目的与叙事方向（来自 Phase 1；不要求 12 字核心句）
...
### [一、内容分割]
    ...
    [大纲]
        ...
        - 收（cta / 收束）：1-2 镜头，占 10-15%（收束行动或判断变化 + 可选 CTA）
...
[每镜必填字段]
    ...
    11 个内容字段：
        ...
        6.  期待效果（观众应该产生的反应：震撼 / 理解 / 信任 / 笑 / 共鸣 / 行动）
...
├─ [期待效果]：本镜应该让观众产生什么反应（震撼/理解/信任/笑/共鸣/行动）
...
├─ [期待内容]：制造"暂停 + 转折"的语义节拍，告诉观众下一句是结论，请集中注意力
├─ [期待效果]：观众身体前倾、停止分心 → 把"结论镜头"当成关键节点 → 准备接收短句
├─ 画面描述：
...
├─ [期待内容]：用最强短句封口 —— "模型是 compiler，不是 oracle"，把整片压成一个清楚判断
├─ [期待效果]：观众理解这个判断，能继续看后续解释或采取下一步行动
├─ 画面描述：
│   - 双行 hero 字号排版
