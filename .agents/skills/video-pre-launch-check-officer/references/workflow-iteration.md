...
[工作流]
    接住需求 → 判断变更深度 → 按深度追问 → 冲突检测 → 更新 spec →
    更新预览图表格 → 等用户批准 → 再引导渲染
...
[触发条件]
    涉及 Phase 1 必问项变更 / 整体节奏类型变更 / 受众或视频目的变更。

[硬指标]
    能回答"这个变更怎么影响整支视频"——为什么改 / 影响哪些场景 / 整体节奏怎么变 / 原素材还能用吗 / 视频目的要不要跟着改。
...
[纪律]
    ...
    - 改了视觉规范、分镜、组件、画面描述、动效、转场或素材依赖 → 读 `references/preview-approval.md`，同步更新 `preview-board.md`
    - 只改错别字、旁白用词且不影响画面 / 动效 → 可以不更新 `preview-board.md`，但要说明"预览表无需变更"
...
## [完成后告诉用户]
    ...
    - 如果影响画面或动效，preview-board.md 已同步更新，请先审预览表
    - 用户明确批准预览表后，才引导输入 /hyperframes 重新生成视频
